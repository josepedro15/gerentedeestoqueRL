-- ============================================================
-- ANÁLISE COMPLETA DE ESTOQUE - VERSÃO CORRIGIDA (NET SALES)
-- ============================================================
-- CORREÇÃO: Removemos ABS() e filtro > 0 para considerar devoluções
-- USANDO DATA DO NODE: {{ $json["data-recebida"] }}
-- ============================================================

WITH 
-- ============================================================
-- CTE 1: Vendas dos últimos 60 dias (LÍQUIDO: Vendas - Devoluções)
-- ============================================================
vendas_60d AS (
    SELECT 
        t2.id_produto, 
        SUM(t2.quantidade) AS qtd_vendida_60d, -- CORRIGIDO: Sem ABS
        SUM(t2.quantidade * t2.preco_unitario) AS faturamento_60d, -- CORRIGIDO: Sem ABS
        MAX(t1.emissao) AS ultima_venda
    FROM v_vendas t1 
    JOIN v_vendas_itens t2 ON t1.id = t2.id_v_vendas
    WHERE t1.emissao >= ('{{ $json["data-recebida"] }}'::date - INTERVAL '60 days')
      -- REMOVIDO: AND t2.quantidade > 0 (Para ler as devoluções)
    GROUP BY t2.id_produto
    HAVING SUM(t2.quantidade) > 0 -- Mantém apenas se o saldo for positivo
),
...
vendas_30d_atual AS (
    SELECT 
        t2.id_produto, 
        SUM(t2.quantidade) AS qtd_vendida_atual,
        SUM(t2.quantidade * t2.preco_unitario) AS faturamento_atual
    FROM v_vendas t1 
    JOIN v_vendas_itens t2 ON t1.id = t2.id_v_vendas
    WHERE t1.emissao >= ('{{ $json["data-recebida"] }}'::date - INTERVAL '30 days')
      -- REMOVIDO: AND t2.quantidade > 0
    GROUP BY t2.id_produto
    HAVING SUM(t2.quantidade) > 0
),

vendas_30d_anterior AS (
    SELECT 
        t2.id_produto, 
        SUM(t2.quantidade) AS qtd_vendida_anterior,
        SUM(t2.quantidade * t2.preco_unitario) AS faturamento_anterior
    FROM v_vendas t1 
    JOIN v_vendas_itens t2 ON t1.id = t2.id_v_vendas
    WHERE t1.emissao >= ('{{ $json["data-recebida"] }}'::date - INTERVAL '60 days')
      AND t1.emissao < ('{{ $json["data-recebida"] }}'::date - INTERVAL '30 days')
      -- REMOVIDO: AND t2.quantidade > 0
    GROUP BY t2.id_produto
    HAVING SUM(t2.quantidade) > 0
),
...
estoque_transito AS (
    SELECT 
        ...
        '{{ $json["data-recebida"] }}'::date - MIN(c.emissao)::date AS dias_aguardando,
...
detalhe_calculado AS (
    SELECT
        ...
        CASE 
            ...
            ELSE ('{{ $json["data-recebida"] }}'::date - v60d.ultima_venda::date)
