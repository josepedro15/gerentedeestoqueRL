...
-- ============================================================
-- Inclui:
-- ✅ Status de Ruptura (dias de cobertura)
-- ✅ Curva ABC (classificação por faturamento)
-- ✅ Margem de Contribuição
-- ✅ Giro de Estoque
-- ✅ Valor do Estoque Parado
-- ✅ Sugestão de Compra
-- ✅ Tendência de Venda (30d vs 30d anteriores)
-- ✅ Dias sem Venda
...
-- ============================================================
-- CTE 3: Estoque agrupado por produto
-- ============================================================
estoque_agrupado AS (
    SELECT 
        id_produto,
...
detalhe_calculado AS (
    SELECT
        ...
        -- MÉDIA DIÁRIA (baseada em 60 dias)
        ...
        -- STATUS DE RUPTURA
        CASE
            WHEN COALESCE(ea.estoque_total, 0) = 0 AND COALESCE(v60d.qtd_vendida_60d, 0) > 0 THEN '🔴 Ruptura'
            WHEN COALESCE(v60d.qtd_vendida_60d, 0) = 0 
                 OR (COALESCE(ea.estoque_total, 0) / (NULLIF(COALESCE(v60d.qtd_vendida_60d, 0), 0) / 60.0)) > 90 THEN '⚪ Excesso'
            WHEN (COALESCE(ea.estoque_total, 0) / (NULLIF(COALESCE(v60d.qtd_vendida_60d, 0), 0) / 60.0)) < 7 THEN '🟠 Crítico'
            WHEN (COALESCE(ea.estoque_total, 0) / (NULLIF(COALESCE(v60d.qtd_vendida_60d, 0), 0) / 60.0)) <= 15 THEN '🟡 Atenção'
            WHEN (COALESCE(ea.estoque_total, 0) / (NULLIF(COALESCE(v60d.qtd_vendida_60d, 0), 0) / 60.0)) <= 90 THEN '🟢 Saudável'
        ...
        -- GIRO DE ESTOQUE (bimestral, convertido para mensal)
        CASE 
        ...
        -- SUGESTÃO DE COMPRA (para 60 dias de cobertura)
        CASE 
        ...
        -- TENDÊNCIA DE VENDA (30d atual vs 30d anterior)
        ...
        -- PRIORIDADE DE COMPRA (combinando ruptura + ABC)
        CASE
            -- Ruptura + Curva A = Máxima urgência
            WHEN COALESCE(ea.estoque_total, 0) = 0 AND COALESCE(v60d.qtd_vendida_60d, 0) > 0 AND COALESCE(abc.classe_abc, 'C') = 'A' THEN '1-URGENTE'
            -- Ruptura + Curva B = Alta prioridade
            WHEN COALESCE(ea.estoque_total, 0) = 0 AND COALESCE(v60d.qtd_vendida_60d, 0) > 0 AND COALESCE(abc.classe_abc, 'C') = 'B' THEN '2-ALTA'
            -- Crítico + Curva A = Alta prioridade
            WHEN (COALESCE(ea.estoque_total, 0) / NULLIF(COALESCE(v60d.qtd_vendida_60d, 0) / 60.0, 0)) < 7 AND COALESCE(abc.classe_abc, 'C') = 'A' THEN '2-ALTA'
            -- Ruptura + Curva C = Avaliar
            WHEN COALESCE(ea.estoque_total, 0) = 0 AND COALESCE(v60d.qtd_vendida_60d, 0) > 0 AND COALESCE(abc.classe_abc, 'C') = 'C' THEN '4-BAIXA'
            -- Crítico + Curva B/C
            WHEN (COALESCE(ea.estoque_total, 0) / NULLIF(COALESCE(v60d.qtd_vendida_60d, 0) / 60.0, 0)) < 7 THEN '3-MEDIA'
            -- Atenção
            WHEN (COALESCE(ea.estoque_total, 0) / NULLIF(COALESCE(v60d.qtd_vendida_60d, 0) / 60.0, 0)) <= 15 THEN '4-BAIXA'
            -- Resto
            ELSE '5-NENHUMA'
        END AS prioridade_compra,
        
        -- ALERTA DE ESTOQUE PARADO (NOVO!)
        CASE
